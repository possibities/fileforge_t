...
| `web_admin/security.py` | 密码哈希、token 生成、token hash、常量时间比较 |
| `web_admin/auth.py` | 登录校验、session 创建/撤销、当前用户加载、权限判断 |
| `web_admin/accounts.py` | 用户管理 service:创建、禁用、重置密码、角色绑定 |
...
| `web_admin/manage.py` | 管理 CLI,最小支持创建/重置平台管理员 |
...
`infrastructure/db/migrations/versions/0003_web_admin_accounts.py`

迁移只新增账户相关表和索引,不重写 0001/0002,不做 destructive 变更。与现有迁移一致,`downgrade()` 抛 `NotImplementedError`。
