...
**输入**:一堆扫描成图片的纸质档案(每个档案一个子目录,目录里是该档案的多页扫描件)。
**输出**:每份档案的结构化元数据(题名、责任者、分类号、保管期限、档号、件号等共 17 个字段),以 JSON + CSV 双格式落盘,可选同时写入 PostgreSQL,并提供 Web 后台供人工查看、过滤、修正。
...
- **Web 后台** 把生成结果呈现给档案员人工核对,允许对 4 个最易错字段做修正,所有修正可追溯
...
- **PostgreSQL 入库**是**旁路**,管线不开 DB 也能跑(`DATABASE_URL` 留空时整条管线行为不变)
- **Web 后台**只读 / 写 DB,**完全不触发管线**(不跑 OCR、不调 LLM)。Web 改了元数据,下一次 `main.py` 跑同一档案时,自动管线会被 `correction_status = "corrected"` 标志保护,不覆盖人工结果
...
启动:`python main.py`(`DATABASE_URL` 已设则同时写库)。
...
### 4.7 阶段 6:数据库旁路写入(`BatchRecorder`,仅当 `DATABASE_URL` 已设)
...
- `processing_batches`:批次汇总(总数/成功数/失败数/失败码统计)
...
- `processing_batches` — 一次 `main.py` 运行对应一个批次
...
**Web 后台账号体系**:
- `organizations` — 单位
- `app_users` — 用户(带密码 pbkdf2 哈希)
- `roles` / `permissions` / `user_roles` / `role_permissions` — 角色权限
...
**读侧 API**(`infrastructure/db/queries.py`):6 个 keyword-only 函数,返回 frozen dataclass,Web + CLI 共用:
...
organizations  ──┐
     ...
     │                       │
   projects                 app_users
     │ 1:N                    │ N:M (roles ↔ permissions)
     ▼
   processing_batches
     │ 1:N
     ▼
   archive_records ──┬──→ archive_pages       (N 页)
                     ├──→ metadata_revisions  (N 次修订)
                     └──→ audit_logs          (action=manual_correction / force_rerun_rules)
...
`tests/` 目录约 30 个测试文件,318 个用例,**全部跑 SQLite in-memory**,不依赖 PaddleOCR / vLLM / PostgreSQL。
...
| `test_*_cli.py` | CLI 子命令(archive_query / force_rerun / user_admin) |
...
期望 318 通过,部分用例若依赖未装(如 fastapi、paddleocr)会自动 `@unittest.skipUnless` 跳过而不是失败。
...
| CLI:archive_query / user_admin / force_rerun | ✅ |
| Web 后台:登录 + 用户/单位/项目管理 + 批次/档案/修订/审计只读 + 4 字段人工修正 | ✅ |
| 权限矩阵(三角色)+ 组织隔离 + CSRF | ✅ |
| 完整测试覆盖(318 用例,SQLite 跑) | ✅ |
...
9. **Web 单向依赖**:`web_admin → infrastructure/db`,Web 层不直接读 ORM 而是走 service 层(accounts/projects/queries 等),便于测试与替换实现
10. **测试用 SQLite in-memory**:CI 不依赖 PG,318 用例几秒跑完;Web 用 `TestClient` 走 ASGI,不开 HTTP socket
...
- **可演示**:跑一遍 `main.py` 出 JSON/CSV,登录 Web 后台演示 4 字段修正、组织隔离、修订历史
