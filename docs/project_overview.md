...
>
> 文档基线:以当前代码为准(`core/`、`processors/`、`infrastructure/db/`、`web_admin/`)。当前会话属于非可执行环境,本文中的运行命令是目标 Linux/GPU/数据库环境中的操作说明,不是本环境已执行结果。
...
│   │  ├─ RulesEngine  (补充规则 + 题名清洗/标记 + 期限锁定)      │  │
│   │  └─ SequenceGenerator (件号/档号分配,2007 年期限码切换)     │  │
│   └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│   ┌──────────────────────────────────────────────────────────────┐  │
...
### 4.3 阶段 2:OCR(`OcrClient.extract_text_from_images`)
...
### 4.5 阶段 4:规则引擎(`RulesEngine.apply_all`)

`core/rules_engine.py` — 严格优先级,从上到下逐条执行:

1. **强制字段修正与合法性校验**:补齐导出保留字段,强制 `档案馆代码`/`档案馆名称` 为空,同步 `立档单位名称`,并校验 `密级` ∈ {非涉密/内部/秘密/机密/绝密}、`保密期限` ∈ {1年/5年/10年};非法值置空。
2. **10 条补充分类/期限规则**(部分举例):
   - rule 2:题名含"简报" → 保管期限锁定为 `10年`,设置 `period_locked=True` 标志
   - 培训类 → 业务类,保管期限至少 `30年`
   - 档案寄存地址变更、安装维修类函/通知、一般事务性通知 → `10年`
   - 内部制度/管理办法/条例等 → 综合类,`30年`
   - 批评通报、中标结果/通知函、党支部调整请示 → `30年`
   - 带有效 `文件编号` 的本单位文件 → 至少 `30年`
   - 已锁定的字段后续规则跳过(避免冲突)
3. **开放状态判定**:优先级 `密级标注 > 个人隐私关键词 > 商业秘密 > 负面信息`,任意一项命中就把 `开放状态` 设为 `控制`,并写 `延期开放理由`;默认 `开放`。
4. **分类号校验**:优先按 `文件形成时间` 年份判断,取不到再用 `归档年度`;2020 年以前必须用旧码(001/002/003),2020 年及以后用新码(DQL/ZHL/YWL)。
5. **题名清洗与标记**:用十余条正则清除日期、期号、文件编号、重复另拟、简报尾部描述等确定性噪声。
...
- **prompt 装配**:`ArchiveClassifier.__init__` 时读 `prompts/` 下:
  - `rules_priority.txt` — 最高优先级补充规则
  - `rules_category.txt` — 分类与保管期限规则
  - `rules_title.txt` — 题名著录规则
  - `rules_openness.txt` — 密级、开放状态与延期开放理由规则
  - `rules_fields.txt` — 字段提取和分类边界补充说明
  - `checklist.txt` — 输出前高频错误检查
...
| 段 | 行数 | 责任 |
| --- | --- | --- |
| `apply_all()` | 入口 | 按优先级编排下列方法 |
| `_force_fix_fields` | 字段修正 | 保留字段、立档单位、密级/保密期限合法性 |
| `_apply_supplementary_rules` | ~200 行 | 10 条补充规则 + 期限锁定 |
| `_apply_open_status_rules` | ~60 行 | 开放状态判定 |
| `_validate_classification_code` | ~30 行 | 新旧码切换 |
| `_clean_title` | ~150 行 | 题名清洗与 rule 11 标记 |
...
- `0005_upload_online_processing` — 重建为上传 + 在线跑批 schema,删除旧应用表后按当前 ORM 重建
...
- `DB_RERUN_POLICY="skip-success"` — `skip-success` / `rerun-failed-only` / `rerun-all`
...
`tests/` 目录覆盖核心规则、批处理、导出、DB、Web 路由、上传和 CLI。测试以 SQLite in-memory 和 stub 为主,不要求真实 PaddleOCR、vLLM 或 PostgreSQL 服务。当前非可执行环境不声明这些测试已在本会话通过。
...
| `test_rules_engine.py` | 补充规则、题名清洗/标记、期限锁定、开放状态和分类号校验 |
...
在可执行开发环境中应以该命令的实际结果为准；不要在未运行测试时声称“全部通过”。
...
│  ├─ train.py
│  └─ data/{train,eval}.json  # 训练数据需自行准备
│
├─ requirements/            # 分层依赖
│  ├─ base.txt              # 管线核心
│  ├─ nvi.txt               # GPU + 微调
...
sudo -u postgres createdb -O fileforge fileforge_current
export DATABASE_URL="postgresql+psycopg://fileforge:PWD@127.0.0.1:5432/fileforge_current"
...
- 角色管理页(Web)— 当前角色由 `app_users.role` 和代码内置权限映射固定,不能在线编辑
- 项目重命名 / 项目转单位
...
10. **测试用 SQLite in-memory**:多数 DB/Web 测试不依赖真实 PostgreSQL;Web 用 `TestClient` 走 ASGI,不开 HTTP socket。当前非可执行环境只做静态审阅,测试通过性需在目标环境运行命令确认
