...
- 导出文件记录。
- 项目内件号计数器。
- Web 后台阶段保存单位、用户、角色、权限和 Web session。
...
- `llm_metadata/rules_metadata/final_metadata` 三阶段快照；完整写入需要调整当前分类器返回结构。
- 人工修正记录。
- 操作审计日志。
- 页面级 OCR 文本和置信度入库。
- 单位、用户、角色、权限基础模型。
...
权限粒度建议先覆盖（`permissions.code` 见数据契约文档 §4.2，下表为代码与中文显示名对照）：

| `permissions.code` | 中文显示名 |
| --- | --- |
...
说明：当前 `batch_summary.json` 的结果状态只有 `success/failed/error`。`pending/running/needs_review/corrected/confirmed` 不应直接写入现有 summary 的 result `status` 字段。批次级 `completed` 不暗示零失败，零失败需查 `fail_count == 0`。
...
- 调整分类器返回结构或增加处理钩子，保存 LLM 结果、规则结果、人工校对结果三个快照。
- 提供人工修正能力。
- 写入 `metadata_revisions`。
- 写入 `audit_logs`。
...
账号、角色、权限、Web session、登录、用户管理、批次/档案/修订/审计只读页面已经在阶段 2 基础后台中部分落地。阶段 3 应在此基础上补齐单位/项目管理、在线修正、导出记录和更完整的 API 边界。

API 层：

- 建立或完善单位、账户、角色、权限和 Web session 表。
...
- 任务调度、限流、取消、重试策略。
- 后台实时进度刷新。
- 队列系统复用 `processing_jobs` 与 `processing_job_attempts`，不重新定义状态模型。
...
3. 一期重点实现显式项目、项目内续号、旁路入库、批次追踪、档案查询、任务尝试和导出记录。
4. metadata 快照、账户权限、人工修正和审计按阶段推进，但数据模型需提前兼容。
