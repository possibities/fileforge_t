...
`archive_records.review_status` 表示审核状态（实现现状）：

- `pending`（待审核）
- `needs_review`（重点审核）
- `reviewed`（已审核）
- `not_required`（无需审核；处理失败档案）
- 兼容保留的历史取值：`in_review`、`confirmed`
...
说明：当前 `batch_summary.json` 的结果状态只有 `success/failed/error`。`pending/running/needs_review/reviewed/corrected` 不应直接写入现有 summary 的 result `status` 字段。批次是否零失败以 `fail_count == 0` 和 `batch_status == success` 判断。
...
- 早期的导出占位字段（`全宗号`、`档案馆代码`、`档案馆名称`、`外包单位名称`）已下线，不再写入 `final_metadata` 或导出列（见数据契约文档 §2.3）。
