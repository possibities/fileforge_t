...
> 实现状态修订说明（阅读前必读）：本文档为 PostgreSQL 集成的**早期设计稿**，最终实现做了如下简化与取舍，下文中涉及这些内容的章节属于设计预案，不代表当前已实现的数据库结构（以 `infrastructure/db/models.py` 为准）：
> 1. **权限模型**：未采用独立的 `roles` / `permissions` / `user_roles` / `role_permissions` 四表，已简化为 `app_users.role` 单字段加代码内置的“角色到权限”映射。
> 2. **处理重试**：未实现 `processing_job_attempts` 表；`processing_jobs` 保存当前状态、进度/阶段、页数、最后错误和尝试次数,历史事件进入 `processing_events`。
> 3. **件号重排**：`force-renumber` 策略未实现；`DB_RERUN_POLICY` 实际仅支持 `skip-success` / `rerun-failed-only` / `rerun-all`。
> 4. **GIN 索引**：`0001` 迁移曾以方言守卫的原始 SQL 为 `final_metadata` 建过 GIN 索引，但 `0005` 重建迁移会删表并按 `models.py` 重新建表，而 `models.py` 未定义 GIN，故当前生效 schema 不含 GIN（仅保留 `archive_no` 的部分唯一索引）。
...
- `pending`（待审核）
- `needs_review`（重点审核，审核队列置顶）
- `reviewed`（已审核）
- `not_required`（无需审核；处理失败/异常档案，不进入审核队列）
- 兼容保留的历史取值：`in_review`、`confirmed`（旧词表，新流程不再写入）

含义（与 `infrastructure/db/recorder.py` 一致）：

- 处理成功且无疑难标记 → `pending`（进入审核队列的常规项）。
- 简报题名二次重写失败等产生 `备注` 待核查提示 → `needs_review`（置顶）。
- 处理失败/异常 → `not_required`，不进入审核队列。
- 档案员在审核工作台确认保存后 → `reviewed`。
- 审核队列（`/review` 与全局 `verification_queue`）= `processing_status='success' AND review_status != 'reviewed'`，即“待审核 + 重点审核”的并集。
...
以下模型面向 Web 管理后台和多人协作；当前实现已落地单位、用户和 Web session 表。权限不落独立 RBAC 表,而是由 `app_users.role` 字段和代码内置角色映射提供。
...
- `app_users.role` 是当前唯一角色字段,取值为 `platform_admin`、`org_admin`、`org_operator`。`infrastructure/db/accounts.py` 根据该字段映射权限；`python -m utils.user_admin roles init` 只是兼容入口,不会写旧 RBAC seed 表。
...
- `queued`：已建批次,等待开始。
- `running`：批次正在执行。
- `success`：批次执行结束且无失败档案。
- `partial_failed`：批次执行结束,部分档案失败或异常。
- `failed`：批次执行结束但无成功档案,或批次级异常。
- `cancelled`：批次被取消。
- 兼容旧值：`completed`、`aborted` 仍保留在 ORM 枚举中,仅用于历史兼容。

注意：`batch_status` 与 `archive_records.processing_status` 解耦。批次是否零失败以 `fail_count == 0` 和 `batch_status == success` 共同判断。
...
- 当前实现状态：`infrastructure/db/repositories.py:upsert_pages` 已通过 `_to_relative_posix(image_path, input_dir)` 存相对 `input_dir` 的 POSIX 路径；无法相对化时退化为原路径的 POSIX 风格字符串。
...
- `archive_id`
- `document_key`
- `processing_status`（ORM 映射到列名 `status`）
- `progress`
- `current_stage`
- `page_count`
- `attempt_count`
...
- `processing_jobs` 保存当前任务状态、进度/阶段、页数、最后错误摘要和 `attempt_count`。
- 当前未实现独立 `processing_job_attempts` 表；每次开始/结束/错误通过 `processing_events` 追加事件,用于 Web 进度页和排错。
- 后续若引入独立队列,可在不改变 OCR/LLM/规则主流程的前提下扩展更细的 attempt 历史。
...
当前实现已经形成三层快照边界。`LlmClient.extract_metadata()` 在 `ExtractionTrace.parsed_metadata` 中保存规则前 LLM 解析快照；`RulesEngine.apply_all()` 仍会就地修正 metadata；`BatchRecorder.on_archive_complete()` 将 LLM 快照、规则后结果和最终结果分别写入 `llm_metadata`、`rules_metadata`、`final_metadata`。
...
- `LlmClient` 已保存 `raw_response`、`cleaned_response`、`parse_strategy` 和 `parsed_metadata`；主抽取 trace 会写入 `archive_records.llm_*` 缓存列和 `llm_traces` 历史表。
- 简报题名二次重写使用独立 `briefing_rewrite` trace 行,不覆盖主抽取缓存列。
- 简报题名二次重写失败写入 `备注` 的待核查提示时,`BatchRecorder._derive_review_status()` 会把 `review_status` 置为 `needs_review`。
- 写入 `final_metadata` 的代码路径已在 `apply_classification_result()` 中检查 `correction_status`，命中 `corrected` 时跳过覆盖（除非显式 `force_rerun_rules=True`）。
...
读取 DATABASE_URL、PROJECT_KEY、BATCH_KEY
  ...
  -> 更新 processing_jobs 并追加 processing_events
  -> 继续生成单档案 JSON、batch_summary.json、汇总 JSON/CSV
  -> 回填 archive_records.result_filename
  -> 写入 export_files
  -> 更新 processing_batches.batch_status（success/partial_failed/failed）
```
...
- 档案大事务：单件 metadata、状态、任务/事件、`archive_records` 与 `archive_pages` 的写入在一个事务中提交。该事务覆盖 OCR/LLM/规则全程，可能持续数十秒到数分钟。
...
- “凭文件补 DB”：以 `output_dir/batch_summary.json` 为入口，遍历同目录下的单档案 JSON，回写 `archive_records / archive_pages / processing_jobs / processing_events`，缺失的件号需走 §4.9 的尾部新发号策略；建议落在 `scripts/replay_files_to_db.py`。
...
- 当前不支持 `force-renumber`。如未来增加,必须作为高风险显式操作并写入审计。
...
当前实现不设 `processing_job_attempts` 表。`processing_jobs.attempt_count` 记录尝试次数,`error_code/error_message` 保存最后错误摘要,`processing_events` 追加阶段开始、结束和错误事件。若未来引入独立 worker 和精细重试历史,再增加 attempt 表。
...
阶段 1C 落地的只读查询 API,供 CLI 与当前 Web 后台共用。代码见 `infrastructure/db/queries.py`。
...
| `list_audit_logs(session, *, target_type, target_id, page=1, page_size=50)` | `ListResult[AuditLogRow]` | 审计日志(`target_type` 当前白名单为 `{"archive", "batch", "project", "user", "organization", "upload"}`) |
...
| `list_audit_logs(target_type` 不在当前白名单内) | 抛 `ValueError` |
...
当前白名单在 `infrastructure/db/queries.py` 中维护,覆盖档案修正、上传、批次、项目、用户和单位等后台操作。新增审计对象时应先扩展该白名单和对应 Web/CLI 测试。
...
阶段 2 Web 后台审核工作台 `/review/{id}/save` 引入一个新写侧函数,与既有 `apply_force_rerun_rules` 正交:

- `infrastructure/db/repositories.py:apply_manual_correction(session, *, archive, new_values, actor_user_id, reason=None) -> int`:供 Web 审核工作台 POST 调用。`new_values`(`ManualCorrectionInput`)接受 8 个字段(`题名` / `责任者` / `实体分类号` / `保管期限` / `开放状态` / `归档年度` / `文件编号` / `立档单位名称`)的新值,其中实体分类名称随分类号自动派生;内部 diff、记 `metadata_revisions` 与 `audit_logs(action="manual_correction")`,把 `correction_status` 置为 `corrected`,并按新值重新派生 `retention_period_code`。函数自身不 commit;无差异返回 0(无副作用)。
- `new_values` 由 `ManualCorrectionInput` dataclass 承载,所有字段在 Web 入口处完成 strip / 长度 / enum 校验(`保管期限` ∈ `{"永久", "30年", "10年"}`),函数本身不二次校验。
- 与 `apply_force_rerun_rules(action="force_rerun_rules")` 互补:后者清掉 `corrected` 强制覆盖整套 metadata,适合 CLI 全字段重跑;前者只动审核工作台开放的 8 个字段并置 `corrected`,适合 Web 端人工微调。两条路径写不同的 `action` 标签,便于审计反查。
...
- 当前文件输出保持不变。
- 成功档案写入数据库并支持基础查询。
- `llm_metadata/rules_metadata/final_metadata` 三快照已完整写入；`final_metadata` 是导出和人工修正后的权威结果。

### 阶段 2：metadata 快照与人工修正

- 继续保持 `llm_metadata`、`rules_metadata`、`final_metadata` 三个快照的写入边界。
- 增加人工修正 API。
...
- 增加单位、用户和 Web session 表；角色权限继续使用 `app_users.role` 加代码内置映射。
...
- 队列系统复用 `processing_jobs` 和 `processing_events`，如确需逐次尝试历史,再增量设计 attempt 表。

## 11. 仍需业务确认的问题

阶段 1 决策已收敛：

1. **同一 `batch_key` 重跑默认行为** → 已定为 `skip-success`（只重跑 failed/error/pending），另支持 `rerun-failed-only` 和 `rerun-all`;当前不支持 `force-renumber`。详见 §7.1。
